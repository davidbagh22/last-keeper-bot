...
async def _is_superadmin(user_id: int) -> bool:
    return game.is_superadmin(user_id)
...
async def admins_panel(callback: CallbackQuery) -> None:
    ...
    rows = await game.db.all(
        '''SELECT u.telegram_id, u.full_name, u.team,
           ...
           ORDER BY is_admin DESC, u.full_name'''.format(','.join('?' for _ in game.settings.superadmin_ids) or '0'),
        tuple(game.settings.superadmin_ids),
    )
    if not rows:
    ...
    await callback.message.answer(
        ...
        'Выбери зарегистрированного пользователя. В следующем окне можно выдать или отозвать доступ. '
        'Суперадминистратор проекта не отображается в списке и не может быть удалён.',
...
async def admin_user_card(callback: CallbackQuery) -> None:
    ...
    await callback.answer()
    action = ('Отозвать права администратора', f'access:remove:{target_id}') if row else ('Выдать права администратора', f'access:add:{target_id}')
    await callback.message.answer(
...
async def add_admin(callback: CallbackQuery, bot: Bot) -> None:
    ...
    await game.db.execute(
        ...
        (target_id, callback.from_user.id, game.utcnow()),
    )
    ...
    try:
        await bot.send_message(
            ...
            'Кнопка «🛡 Управление проектом» теперь будет постоянно отображаться в главном меню. '
            'Открой /start, чтобы обновить меню.',
...
async def remove_admin(callback: CallbackQuery, bot: Bot) -> None:
    ...
    target_id = int(callback.data.rsplit(':', 1)[1])
    if target_id in game.settings.superadmin_ids:
        await callback.answer('Суперадминистратора нельзя удалить.', show_alert=True)
        return
    user = await game.get_user(target_id)
